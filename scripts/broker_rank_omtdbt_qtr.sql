...
--drop table dibots_v2.broker_rank_omtdbt_qtr;
create table dibots_v2.broker_rank_omtdbt_qtr (
year int,
qtr int,
year_qtr varchar(10),
...
alter table dibots_v2.broker_rank_omtdbt_qtr add constraint broker_rank_omtdbt_qtr_pk primary key (year, qtr, participant_code);
create index broker_rank_omtdbt_qtr_ext_id_idx on dibots_v2.broker_rank_omtdbt_qtr (external_id);

select * from dibots_v2.broker_rank_omtdbt_qtr order by year desc, qtr desc

-- FRESH INSERT
insert into dibots_v2.broker_rank_omtdbt_qtr(year, qtr, participant_code, total_vol, total_val, local_vol, local_val, foreign_vol, foreign_val,
...
select year, qtr, participant_code, sum(total_vol), sum(total_val), sum(local_vol), sum(local_val), sum(foreign_vol), sum(foreign_val),
...
group by year, qtr, participant_code

-- INSERT THE OMT DATA
insert into dibots_v2.broker_rank_omtdbt_qtr (year, qtr, year_qtr, trading_day_count, external_id, participant_code, participant_name, 
...
select year, qtr, year_qtr, trading_day_count, external_id, participant_code, participant_name, 
...
from 
(select year, qtr, participant_code, count(*) as day_count from dibots_v2.broker_rank_omtdbt_day
...
--drop table dibots_v2.broker_rank_omtdbt_qtr_totals;
create table dibots_v2.broker_rank_omtdbt_qtr_totals (
year int,
qtr int,
...
insert into dibots_v2.broker_rank_omtdbt_qtr_totals (year, qtr, year_qtr, total_vol, total_val, local_vol, local_val, local_pct, foreign_vol, foreign_val, foreign_pct, inst_vol, inst_val, inst_pct,
...
select year, qtr, year_qtr, sum(total_vol), sum(total_val), 
...
foreign_retail_vol, foreign_retail_val, foreign_retail_pct, nominees_vol, nominees_val, nominees_pct, local_nominees_vol, local_nominees_val, local_nominees_pct, foreign_nominees_vol,
foreign_nominees_val, foreign_nominees_pct, prop_vol, prop_val, prop_pct, ivt_vol, ivt_val, ivt_pct, pdt_vol, pdt_val, pdt_pct)
select year, qtr, year_qtr, total_trade_volume, total_trade_value, local_trade_volume, local_trade_value, local_trade_pct, foreign_trade_volume, foreign_trade_value, foreign_trade_pct, 
...
from dibots_v2.broker_rank_qtr_totals
where year_qtr < '2020-1'
...
where a.year = b.year and a.qtr = b.qtr and a.algo_count is null;
...
select year, qtr, sum(total_pct), sum(local_pct), sum(foreign_pct), sum(nominees_pct), sum(local_nominees_pct), sum(foreign_nominees_pct), sum(inst_pct), sum(local_inst_pct), sum(foreign_inst_pct), 
...
rank_total = tmp.rank
from
(select year, qtr, participant_code, rank() over (partition by year, qtr order by total_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where total_val <> 0
group by year, qtr, participant_code, total_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
rank_local = tmp.rank
from
(select year, qtr, participant_code, rank() over (partition by year, qtr order by local_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where local_val <> 0
group by year, qtr, participant_code, local_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
from
(select year, qtr, participant_code, rank() over (partition by year, qtr order by foreign_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where foreign_val <> 0
group by year, qtr, participant_code, foreign_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
rank_inst = tmp.rank
from
(select year, qtr, participant_code, rank() over (partition by year, qtr order by inst_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where inst_val <> 0
group by year, qtr, participant_code, inst_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
from
(select year, qtr, participant_code, rank() over (partition by year, qtr order by local_inst_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where local_inst_val <> 0
group by year, qtr, participant_code, local_inst_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
from
(select year, qtr, participant_code, rank() over (partition by year, qtr order by foreign_inst_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where foreign_inst_val <> 0
group by year, qtr, participant_code, foreign_inst_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
from
(select year, qtr, participant_code, rank() over (partition by year, qtr order by retail_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where retail_val <> 0
group by year, qtr, participant_code, retail_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
from
(select year, qtr, participant_code, rank() over (partition by year, qtr order by local_retail_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where local_retail_val <> 0
group by year, qtr, participant_code, local_retail_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
from
(select year, qtr, participant_code, rank() over (partition by year, qtr order by foreign_retail_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where foreign_retail_val <> 0
group by year, qtr, participant_code, foreign_retail_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
from
(select year, qtr, participant_code, rank() over (partition by year, qtr order by nominees_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where nominees_val <> 0
group by year, qtr, participant_code, nominees_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
from
(select year, qtr, participant_code, rank() over (partition by year, qtr order by local_nominees_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where local_nominees_val <> 0
group by year, qtr, participant_code, local_nominees_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
from
(select year, qtr, participant_code, rank() over (partition by year, qtr order by foreign_nominees_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where foreign_nominees_val <> 0
group by year, qtr, participant_code, foreign_nominees_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
rank_prop = tmp.rank
from
(select year, qtr, participant_code, rank() over (partition by year, qtr order by prop_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where prop_val <> 0
group by year, qtr, participant_code, prop_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
rank_ivt = tmp.rank
from
(select year, qtr, participant_code, rank() over (partition by year, qtr order by ivt_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where ivt_val <> 0
group by year, qtr, participant_code, ivt_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
rank_pdt = tmp.rank
from
(select year, qtr, participant_code, rank() over (partition by year, qtr order by pdt_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where pdt_val <> 0
group by year, qtr, participant_code, pdt_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
delete from dibots_v2.broker_rank_omtdbt_qtr where year_qtr = (select max(year_qtr) from dibots_v2.broker_rank_omtdbt_qtr);
...
sum(prop_vol), sum(prop_val), sum(ivt_vol), sum(ivt_val), sum(pdt_vol), sum(pdt_val)
from dibots_v2.broker_rank_omtdbt_day
where year >= (select max(year) from dibots_v2.broker_rank_omtdbt_qtr) and 
qtr > (select max(qtr) from dibots_v2.broker_rank_omtdbt_qtr where year = (select max(year) from dibots_v2.broker_rank_omtdbt_qtr))
group by year, qtr, participant_code;
...
(select year, qtr, participant_code, count(*) as day_count from dibots_v2.broker_rank_omtdbt_day
group by year, qtr, participant_code) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code and a.trading_day_count is null;

-- Get the ALGOs from broker_rank_qtr table
update dibots_v2.broker_rank_omtdbt_qtr a
set
algo_count = b.algo_count,
algo_pct = b.algo_pct,
...
ann_velocity = b.ann_velocity
from dibots_v2.broker_rank_qtr b
where a.year_qtr = b.year_qtr and a.participant_code = b.participant_code and a.year >= ? and a.qtr >= ?;

-- TOTALS

-- DELETE LATEST FIRST
select max(year_qtr) from dibots_v2.broker_rank_omtdbt_qtr_totals;

delete from dibots_v2.broker_rank_omtdbt_qtr_totals where year_qtr = (select max(year_qtr) from dibots_v2.broker_rank_omtdbt_qtr_totals);
...
sum(pdt_vol), sum(pdt_val), sum(pdt_val) / sum(total_val) * 100
from dibots_v2.broker_rank_omtdbt_qtr
where year_qtr > (select max(year_qtr) from dibots_v2.broker_rank_omtdbt_qtr_totals)
group by year, qtr, year_qtr;
...
pdt_pct = a.pdt_val / b.pdt_val * 100
from dibots_v2.broker_rank_omtdbt_qtr_totals b
where a.year = b.year and a.qtr = b.qtr and a.year_qtr >= (select max(year_qtr) from dibots_v2.broker_rank_omtdbt_qtr);
...
(select year, qtr, participant_code, rank() over (partition by year, qtr order by total_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where total_val <> 0 and year_qtr >= (select max(year_qtr) from dibots_v2.broker_rank_omtdbt_qtr)
group by year, qtr, participant_code, total_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
(select year, qtr, participant_code, rank() over (partition by year, qtr order by local_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where local_val <> 0 and year_qtr >= (select max(year_qtr) from dibots_v2.broker_rank_omtdbt_qtr)
group by year, qtr, participant_code, local_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
(select year, qtr, participant_code, rank() over (partition by year, qtr order by foreign_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where foreign_val <> 0 and year_qtr >= (select max(year_qtr) from dibots_v2.broker_rank_omtdbt_qtr)
group by year, qtr, participant_code, foreign_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
(select year, qtr, participant_code, rank() over (partition by year, qtr order by inst_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where inst_val <> 0 and year_qtr >= (select max(year_qtr) from dibots_v2.broker_rank_omtdbt_qtr)
group by year, qtr, participant_code, inst_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
(select year, qtr, participant_code, rank() over (partition by year, qtr order by local_inst_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where local_inst_val <> 0 and year_qtr >= (select max(year_qtr) from dibots_v2.broker_rank_omtdbt_qtr)
group by year, qtr, participant_code, local_inst_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
(select year, qtr, participant_code, rank() over (partition by year, qtr order by foreign_inst_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where foreign_inst_val <> 0 and year_qtr >= (select max(year_qtr) from dibots_v2.broker_rank_omtdbt_qtr)
group by year, qtr, participant_code, foreign_inst_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
(select year, qtr, participant_code, rank() over (partition by year, qtr order by retail_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where retail_val <> 0 and year_qtr >= (select max(year_qtr) from dibots_v2.broker_rank_omtdbt_qtr)
group by year, qtr, participant_code, retail_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
(select year, qtr, participant_code, rank() over (partition by year, qtr order by local_retail_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where local_retail_val <> 0 and year_qtr >= (select max(year_qtr) from dibots_v2.broker_rank_omtdbt_qtr)
group by year, qtr, participant_code, local_retail_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
(select year, qtr, participant_code, rank() over (partition by year, qtr order by foreign_retail_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where foreign_retail_val <> 0 and year_qtr >= (select max(year_qtr) from dibots_v2.broker_rank_omtdbt_qtr)
group by year, qtr, participant_code, foreign_retail_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
(select year, qtr, participant_code, rank() over (partition by year, qtr order by nominees_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where nominees_val <> 0 and year_qtr >= (select max(year_qtr) from dibots_v2.broker_rank_omtdbt_qtr)
group by year, qtr, participant_code, nominees_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
(select year, qtr, participant_code, rank() over (partition by year, qtr order by local_nominees_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where local_nominees_val <> 0 and year_qtr >= (select max(year_qtr) from dibots_v2.broker_rank_omtdbt_qtr)
group by year, qtr, participant_code, local_nominees_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
(select year, qtr, participant_code, rank() over (partition by year, qtr order by foreign_nominees_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where foreign_nominees_val <> 0 and year_qtr >= (select max(year_qtr) from dibots_v2.broker_rank_omtdbt_qtr)
group by year, qtr, participant_code, foreign_nominees_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
(select year, qtr, participant_code, rank() over (partition by year, qtr order by prop_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where prop_val <> 0 and year_qtr >= (select max(year_qtr) from dibots_v2.broker_rank_omtdbt_qtr)
group by year, qtr, participant_code, prop_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
(select year, qtr, participant_code, rank() over (partition by year, qtr order by ivt_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where ivt_val <> 0 and year_qtr >= (select max(year_qtr) from dibots_v2.broker_rank_omtdbt_qtr)
group by year, qtr, participant_code, ivt_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;
...
(select year, qtr, participant_code, rank() over (partition by year, qtr order by pdt_val desc) rank
from dibots_v2.broker_rank_omtdbt_qtr
where pdt_val <> 0 and year_qtr >= (select max(year_qtr) from dibots_v2.broker_rank_omtdbt_qtr)
group by year, qtr, participant_code, pdt_val) tmp
where a.year = tmp.year and a.qtr = tmp.qtr and a.participant_code = tmp.participant_code;

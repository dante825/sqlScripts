...
--drop table dibots_v2.broker_rank_omtdbt_month;
create table dibots_v2.broker_rank_omtdbt_month (
year int,
month int,
...
alter table dibots_v2.broker_rank_omtdbt_month add constraint broker_rank_omtdbt_month_pk primary key (year, month, participant_code);
create index broker_rank_omtdbt_month_ext_id_idx on dibots_v2.broker_rank_omtdbt_month (external_id);

select * from dibots_v2.broker_rank_omtdbt_month order by year desc, month desc
...
-- INSERT OMT DATA
insert into dibots_v2.broker_rank_omtdbt_month (year, month, year_month, trading_day_count, external_id, participant_code, participant_name, 
...
select year, month, year_month, trading_day_count, external_id, participant_code, participant_name, 
...
--drop table dibots_v2.broker_rank_omtdbt_month_totals;
create table dibots_v2.broker_rank_omtdbt_month_totals (
year int,
month int,
...
foreign_retail_vol, foreign_retail_val, foreign_retail_pct, nominees_vol, nominees_val, nominees_pct, local_nominees_vol, local_nominees_val, local_nominees_pct, foreign_nominees_vol,
foreign_nominees_val, foreign_nominees_pct, prop_vol, prop_val, prop_pct, ivt_vol, ivt_val, ivt_pct, pdt_vol, pdt_val, pdt_pct)
select year, month, year_month, total_trade_volume, total_trade_value, local_trade_volume, local_trade_value, local_trade_pct, foreign_trade_volume, foreign_trade_value, foreign_trade_pct, 
...
from dibots_v2.broker_rank_month_totals
where year_month < '2020-01'
...
select year, month, sum(total_pct), sum(local_pct), sum(foreign_pct), sum(nominees_pct), sum(local_nominees_pct), sum(foreign_nominees_pct), sum(inst_pct), sum(local_inst_pct), sum(foreign_inst_pct), 
...
delete from dibots_v2.broker_rank_omtdbt_month where year_month = (select max(year_month) from dibots_v2.broker_rank_omtdbt_month);
...
sum(prop_vol), sum(prop_val), sum(ivt_vol), sum(ivt_val), sum(pdt_vol), sum(pdt_val)
from dibots_v2.broker_rank_omtdbt_day
where year >= (select max(year) from dibots_v2.broker_rank_omtdbt_month) and 
month > (select max(month) from dibots_v2.broker_rank_omtdbt_month where year = (select max(year) from dibots_v2.broker_rank_omtdbt_month))
group by year, month, participant_code;
...
where a.year_month = b.year_month and a.participant_code = b.participant_code and a.year >= '?' and a.month >= '?';

-- TOTALS

-- DELETE LATEST FIRST
select max(year_month) from dibots_v2.broker_rank_omtdbt_month_totals;

delete from dibots_v2.broker_rank_omtdbt_month_totals where year_month = (select max(year_month) from dibots_v2.broker_rank_omtdbt_month_totals);
...
sum(pdt_vol), sum(pdt_val), sum(pdt_val) / sum(total_val) * 100
from dibots_v2.broker_rank_omtdbt_month
where year_month > (select max(year_month) from dibots_v2.broker_rank_omtdbt_month_totals)
group by year, month, year_month;
...
pdt_pct = a.pdt_val / b.pdt_val * 100
from dibots_v2.broker_rank_omtdbt_month_totals b
where a.year = b.year and a.month = b.month and a.year_month >= (select max(year_month) from dibots_v2.broker_rank_omtdbt_month);
...
(select year, month, participant_code, rank() over (partition by year, month order by total_val desc) rank
from dibots_v2.broker_rank_omtdbt_month
where total_val <> 0 and year_month >= (select max(year_month) from dibots_v2.broker_rank_omtdbt_month)
group by year, month, participant_code, total_val) tmp
where a.year = tmp.year and a.month = tmp.month and a.participant_code = tmp.participant_code;
...
(select year, month, participant_code, rank() over (partition by year, month order by local_val desc) rank
from dibots_v2.broker_rank_omtdbt_month
where local_val <> 0 and year_month >= (select max(year_month) from dibots_v2.broker_rank_omtdbt_month)
group by year, month, participant_code, local_val) tmp
where a.year = tmp.year and a.month = tmp.month and a.participant_code = tmp.participant_code;
...
(select year, month, participant_code, rank() over (partition by year, month order by foreign_val desc) rank
from dibots_v2.broker_rank_omtdbt_month
where foreign_val <> 0 and year_month >= (select max(year_month) from dibots_v2.broker_rank_omtdbt_month)
group by year, month, participant_code, foreign_val) tmp
where a.year = tmp.year and a.month = tmp.month and a.participant_code = tmp.participant_code;
...
(select year, month, participant_code, rank() over (partition by year, month order by inst_val desc) rank
from dibots_v2.broker_rank_omtdbt_month
where inst_val <> 0 and year_month >= (select max(year_month) from dibots_v2.broker_rank_omtdbt_month)
group by year, month, participant_code, inst_val) tmp
where a.year = tmp.year and a.month = tmp.month and a.participant_code = tmp.participant_code;
...
(select year, month, participant_code, rank() over (partition by year, month order by local_inst_val desc) rank
from dibots_v2.broker_rank_omtdbt_month
where local_inst_val <> 0 and year_month >= (select max(year_month) from dibots_v2.broker_rank_omtdbt_month)
group by year, month, participant_code, local_inst_val) tmp
where a.year = tmp.year and a.month = tmp.month and a.participant_code = tmp.participant_code;
...
(select year, month, participant_code, rank() over (partition by year, month order by foreign_inst_val desc) rank
from dibots_v2.broker_rank_omtdbt_month
where foreign_inst_val <> 0 and year_month >= (select max(year_month) from dibots_v2.broker_rank_omtdbt_month)
group by year, month, participant_code, foreign_inst_val) tmp
where a.year = tmp.year and a.month = tmp.month and a.participant_code = tmp.participant_code;
...
(select year, month, participant_code, rank() over (partition by year, month order by retail_val desc) rank
from dibots_v2.broker_rank_omtdbt_month
where retail_val <> 0 and year_month >= (select max(year_month) from dibots_v2.broker_rank_omtdbt_month)
group by year, month, participant_code, retail_val) tmp
where a.year = tmp.year and a.month = tmp.month and a.participant_code = tmp.participant_code;
...
(select year, month, participant_code, rank() over (partition by year, month order by local_retail_val desc) rank
from dibots_v2.broker_rank_omtdbt_month
where local_retail_val <> 0 and year_month >= (select max(year_month) from dibots_v2.broker_rank_omtdbt_month)
group by year, month, participant_code, local_retail_val) tmp
where a.year = tmp.year and a.month = tmp.month and a.participant_code = tmp.participant_code;
...
(select year, month, participant_code, rank() over (partition by year, month order by foreign_retail_val desc) rank
from dibots_v2.broker_rank_omtdbt_month
where foreign_retail_val <> 0 and year_month >= (select max(year_month) from dibots_v2.broker_rank_omtdbt_month)
group by year, month, participant_code, foreign_retail_val) tmp
where a.year = tmp.year and a.month = tmp.month and a.participant_code = tmp.participant_code;
...
(select year, month, participant_code, rank() over (partition by year, month order by nominees_val desc) rank
from dibots_v2.broker_rank_omtdbt_month
where nominees_val <> 0 and year_month >= (select max(year_month) from dibots_v2.broker_rank_omtdbt_month)
group by year, month, participant_code, nominees_val) tmp
where a.year = tmp.year and a.month = tmp.month and a.participant_code = tmp.participant_code;
...
(select year, month, participant_code, rank() over (partition by year, month order by local_nominees_val desc) rank
from dibots_v2.broker_rank_omtdbt_month
where local_nominees_val <> 0 and year_month >= (select max(year_month) from dibots_v2.broker_rank_omtdbt_month)
group by year, month, participant_code, local_nominees_val) tmp
where a.year = tmp.year and a.month = tmp.month and a.participant_code = tmp.participant_code;
...
(select year, month, participant_code, rank() over (partition by year, month order by foreign_nominees_val desc) rank
from dibots_v2.broker_rank_omtdbt_month
where foreign_nominees_val <> 0 and year_month >= (select max(year_month) from dibots_v2.broker_rank_omtdbt_month)
group by year, month, participant_code, foreign_nominees_val) tmp
where a.year = tmp.year and a.month = tmp.month and a.participant_code = tmp.participant_code;
...
(select year, month, participant_code, rank() over (partition by year, month order by prop_val desc) rank
from dibots_v2.broker_rank_omtdbt_month
where prop_val <> 0 and year_month >= (select max(year_month) from dibots_v2.broker_rank_omtdbt_month)
group by year, month, participant_code, prop_val) tmp
where a.year = tmp.year and a.month = tmp.month and a.participant_code = tmp.participant_code;
...
(select year, month, participant_code, rank() over (partition by year, month order by ivt_val desc) rank
from dibots_v2.broker_rank_omtdbt_month
where ivt_val <> 0 and year_month >= (select max(year_month) from dibots_v2.broker_rank_omtdbt_month)
group by year, month, participant_code, ivt_val) tmp
where a.year = tmp.year and a.month = tmp.month and a.participant_code = tmp.participant_code;
...
(select year, month, participant_code, rank() over (partition by year, month order by pdt_val desc) rank
from dibots_v2.broker_rank_omtdbt_month
where pdt_val <> 0 and year_month >= (select max(year_month) from dibots_v2.broker_rank_omtdbt_month)
group by year, month, participant_code, pdt_val) tmp
where a.year = tmp.year and a.month = tmp.month and a.participant_code = tmp.participant_code;

...
--drop table if exists dibots_v2.broker_rank_year;
create table dibots_v2.broker_rank_year (
year int,
...
alter table dibots_v2.broker_rank_year add constraint broker_rank_year_pkey primary key (year, participant_code);
create index broker_rank_year_ext_id_idx on dibots_v2.broker_rank_year (external_id);

insert into dibots_v2.broker_rank_year(year, external_id, participant_code, total_trade_volume, total_trade_value, local_trade_volume, local_trade_value, foreign_trade_volume, foreign_trade_value, nominee_total_trade_volume, nominee_total_trade_value, nominee_local_trade_volume, nominee_local_trade_value,
...
select year,external_id, participant_code, sum(total_trade_volume), sum(total_trade_value), sum(local_trade_volume), sum(local_trade_value), sum(foreign_trade_volume), sum(foreign_trade_value),
...
group by year, external_id, participant_code
...
from
(select year, participant_code, count(*) as day_count from dibots_v2.broker_rank_day
...
from
(select year, participant_code, sum(market_cap) as mart_cap
from dibots_v2.broker_rank_day
where participant_code = 73 -- random cuz if sum all market_cap value would be wrong
group by year, participant_code) tmp
where a.year = tmp.year;

-- ann_velocity
update dibots_v2.broker_rank_year
set
...
drop table dibots_v2.broker_rank_year_totals;
create table dibots_v2.broker_rank_year_totals (
...
insert into dibots_v2.broker_rank_year_totals (year, total_trade_volume, total_trade_value, local_trade_volume, local_trade_value, local_trade_pct, foreign_trade_volume, foreign_trade_value, foreign_trade_pct,
...
select year, sum(total_trade_volume), sum(total_trade_value), 
...
update dibots_v2.broker_rank_year_totals
set
...
select * from dibots_v2.broker_rank_year_totals order by year desc
...
select year, participant_code, participant_name, rank() over (partition by year order by total_trade_value desc) rank
from dibots_v2.broker_rank_year
group by year, participant_code, participant_name, total_trade_value
...
rank_total = tmp.rank
from
(select year, participant_code, rank() over (partition by year order by total_trade_value desc) rank
from dibots_v2.broker_rank_year
group by year, participant_code, total_trade_value) tmp
where a.year = tmp.year and a.participant_code = tmp.participant_code;
...
rank_local = tmp.rank
from
(select year, participant_code, rank() over (partition by year order by local_trade_value desc) rank
from dibots_v2.broker_rank_year
group by year, participant_code, local_trade_value) tmp
where a.year = tmp.year and a.participant_code = tmp.participant_code;
...
from
(select year, participant_code, rank() over (partition by year order by foreign_trade_value desc) rank
from dibots_v2.broker_rank_year
where foreign_trade_value is not null
group by year, participant_code, foreign_trade_value) tmp
where a.year = tmp.year and a.participant_code = tmp.participant_code;
...
from
(select year, participant_code, rank() over (partition by year order by nominee_total_trade_value desc) rank
from dibots_v2.broker_rank_year
where nominee_total_trade_value is not null
group by year, participant_code, nominee_total_trade_value) tmp
where a.year = tmp.year and a.participant_code = tmp.participant_code;
...
from
(select year, participant_code, rank() over (partition by year order by nominee_local_trade_value desc) rank
from dibots_v2.broker_rank_year
where nominee_local_trade_value is not null
group by year, participant_code, nominee_local_trade_value) tmp
where a.year = tmp.year and a.participant_code = tmp.participant_code;
...
from
(select year, participant_code, rank() over (partition by year order by nominee_foreign_trade_value desc) rank
from dibots_v2.broker_rank_year
where nominee_foreign_trade_value is not null
group by year, participant_code, nominee_foreign_trade_value) tmp
where a.year = tmp.year and a.participant_code = tmp.participant_code;
...
from
(select year, participant_code, rank() over (partition by year order by inst_total_trade_value desc) rank
from dibots_v2.broker_rank_year
where inst_total_trade_value is not null
group by year, participant_code, inst_total_trade_value) tmp
where a.year = tmp.year and a.participant_code = tmp.participant_code;
...
from
(select year, participant_code, rank() over (partition by year order by inst_local_trade_value desc) rank
from dibots_v2.broker_rank_year
where inst_local_trade_value is not null
group by year, participant_code, inst_local_trade_value) tmp
where a.year = tmp.year and a.participant_code = tmp.participant_code;
...
from
(select year, participant_code, rank() over (partition by year order by inst_foreign_trade_value desc) rank
from dibots_v2.broker_rank_year
where inst_foreign_trade_value is not null
group by year, participant_code, inst_foreign_trade_value) tmp
where a.year = tmp.year and a.participant_code = tmp.participant_code;
...
from
(select year, participant_code, rank() over (partition by year order by retail_total_trade_value desc) rank
from dibots_v2.broker_rank_year
where retail_total_trade_value is not null
group by year, participant_code, retail_total_trade_value) tmp
where a.year = tmp.year and a.participant_code = tmp.participant_code;
...
from
(select year, participant_code, rank() over (partition by year order by retail_local_trade_value desc) rank
from dibots_v2.broker_rank_year
where retail_local_trade_value is not null
group by year, participant_code, retail_local_trade_value) tmp
where a.year = tmp.year and a.participant_code = tmp.participant_code;
...
from
(select year, participant_code, rank() over (partition by year order by retail_foreign_trade_value desc) rank
from dibots_v2.broker_rank_year
where retail_foreign_trade_value is not null
group by year, participant_code, retail_foreign_trade_value) tmp
where a.year = tmp.year and a.participant_code = tmp.participant_code;
...
from
(select year, participant_code, rank() over (partition by year order by proprietary_trade_value desc) rank
from dibots_v2.broker_rank_year
where proprietary_trade_value is not null
group by year, participant_code, proprietary_trade_value) tmp
where a.year = tmp.year and a.participant_code = tmp.participant_code;
...
rank_ivt = tmp.rank
from
(select year, participant_code, rank() over (partition by year order by ivt_trade_value desc) rank
from dibots_v2.broker_rank_year
where ivt_trade_value is not null
group by year, participant_code, ivt_trade_value) tmp
where a.year = tmp.year and a.participant_code = tmp.participant_code;
...
rank_pdt = tmp.rank
from
(select year, participant_code, rank() over (partition by year order by pdt_trade_value desc) rank
from dibots_v2.broker_rank_year
where pdt_trade_value is not null
group by year, participant_code, pdt_trade_value) tmp
where a.year = tmp.year and a.participant_code = tmp.participant_code;
...
rank_algo = tmp.rank
from
(select year, participant_code, rank() over (partition by year order by algo_count desc) rank
from dibots_v2.broker_rank_year
where algo_count is not null
group by year, participant_code, algo_count) tmp
where a.year = tmp.year and a.participant_code = tmp.participant_code;
...
rank_inet = tmp.rank
from
(select year, participant_code, rank() over (partition by year order by inet_count desc) rank
from dibots_v2.broker_rank_year
where inet_count is not null
group by year, participant_code, inet_count) tmp
where a.year = tmp.year and a.participant_code = tmp.participant_code;
...
rank_dma = tmp.rank
from
(select year, participant_code, rank() over (partition by year order by dma_count desc) rank
from dibots_v2.broker_rank_year
where dma_count is not null
group by year, participant_code, dma_count) tmp
where a.year = tmp.year and a.participant_code = tmp.participant_code;
...
rank_brok = tmp.rank
from
(select year, participant_code, rank() over (partition by year order by brok_count desc) rank
from dibots_v2.broker_rank_year
where brok_count is not null
group by year, participant_code, brok_count) tmp
where a.year = tmp.year and a.participant_code = tmp.participant_code;

--========

select * from dibots_v2.broker_rank_year where year = 2020